...
GLuint Shader::uniformLocation(const char *uniform)
{
...
bool Shader::LoadShaders(const char * vertex_file_path,const char * fragment_file_path){
	...
	if(VertexShaderStream.is_open()){
		...
		qCritical()<<QString("Impossible to open ")+vertex_file_path;
		return false;
	}
	...
	if ( InfoLogLength > 1 ){
		...
		qWarning()<<&VertexShaderErrorMessage[0];
	}
	...
	if ( InfoLogLength > 1 ){
		...
		qWarning()<<&FragmentShaderErrorMessage[0];
	}
	...
	if ( InfoLogLength > 1 ){
		...
		qWarning()<<&ProgramErrorMessage[0];
	}
