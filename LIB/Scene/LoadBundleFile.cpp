	...
	*-----------------------------------------------------------------------------*/
	...
	try{
		...
		*-----------------------------------------------------------------------------*/
		...
		for(unsigned int i=0;i<use_images.size();i++)
			if(use_images[i]<=(unsigned int) numCamera && PhotoFlyXmlManager::getCamInf(use_images[i]).used )
...
std::vector< std::shared_ptr<Camera> > loadBundleFileHDR(bool useAll,bool loadImage) {
	...
	*-----------------------------------------------------------------------------*/
	...
	if ( !bundle_file.is_open()){
		throw std::exception ("BAKA: Load a bundle file !!");
	}
	...
	*-----------------------------------------------------------------------------*/
	std::vector<bool> use_flag;
	std::vector<unsigned int> use_images = CConfigLib::getImagesSet();
	numCamera=(int) PhotoFlyXmlManager::getCamInf().size();
