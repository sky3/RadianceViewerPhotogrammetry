...
unsigned int   Camera::numMVSPnts (void)          const { return (unsigned int) _mvsPnts.size(); }
...
void Camera::resize(float scale){
	...
	_w  = (unsigned int) std::floor( (float)scale*_w );
	_h  = (unsigned int) std::floor( (float)scale*_h );
